...
- (void)addButtonsToView: (UIView *)container
{
    ...
    for (int i=0; i<[buttonTitles count]; i++) {
        ...
        // There is a vertical line between buttons
        if (i == [buttonTitles count] - 1) {
            continue;
        }
        UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(closeButton.frame.origin.x + closeButton.frame.size.width, closeButton.frame.origin.y, 0.5, closeButton.frame.size.height)];
        lineView.backgroundColor = [UIColor colorWithRed:198.0/255.0 green:198.0/255.0 blue:198.0/255.0 alpha:1.0f];
